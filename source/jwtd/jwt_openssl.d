...
version(UseOpenSSL) {
	...
	string sign(string msg, string key, JWTAlgorithm algo = JWTAlgorithm.HS256) {
		...
		void sign_hs(const(EVP_MD)* evp, uint signLen) {
            ...
            version (UseOpenSSL10) {
               HMAC_CTX ctx;
               HMAC_CTX *pctx = &ctx;
               scope(exit) HMAC_CTX_cleanup(pctx);
               HMAC_CTX_init(pctx);
            }
            version (UseOpenSSL11) {
                HMAC_CTX *pctx = HMAC_CTX_new();
                scope(exit) HMAC_CTX_free(pctx);
            }

			if(0 == HMAC_Init_ex(pctx, key.ptr, cast(int)key.length, evp, null)) {
				throw new Exception("Can't initialize HMAC context.");
			}
			if(0 == HMAC_Update(pctx, cast(const(ubyte)*)msg.ptr, cast(ulong)msg.length)) {
				throw new Exception("Can't update HMAC.");
			}
			if(0 == HMAC_Final(pctx, cast(ubyte*)sign.ptr, &signLen)) {
